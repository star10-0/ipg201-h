...
namespace TaskManagementSystem
{
    public partial class CreateAccountForm : Form
    {
        ...
        {
            Text = "إنشاء حساب"; Size = new Size(420, 340);
            ...
            var btn = new Button { Text = "إنشاء الحساب", Top = 240, Left = 160, Width = 100 };
            Controls.AddRange(new Control[] { new Label { Text = "اسم الموظف", Top = 30, Left = 20 }, txtName, new Label { Text = "كلمة المرور", Top = 70, Left = 20 }, txtPass, new Label { Text = "القسم", Top = 110, Left = 20 }, cmbDept, new Label { Text = "رقم الموظف", Top = 150, Left = 20 }, txtNo, new Label { Text = "البريد الإلكتروني", Top = 190, Left = 20 }, txtEmail, btn });
            btn.Click += (s, e) =>
            {
                if (string.IsNullOrWhiteSpace(txtName.Text) || string.IsNullOrWhiteSpace(txtPass.Text) || cmbDept.SelectedItem == null || string.IsNullOrWhiteSpace(txtNo.Text)) { MessageBox.Show("جميع الحقول المطلوبة يجب تعبئتها."); return; }
                var ok = _authService.CreateAccount(txtName.Text, txtPass.Text, cmbDept.SelectedItem.ToString()!, txtNo.Text, txtEmail.Text);
                MessageBox.Show(ok ? "تم إنشاء الحساب بنجاح." : "رقم الموظف مكرر أو حدث خطأ في الحفظ.");
                if (ok) Close();
            };
        }
    }
